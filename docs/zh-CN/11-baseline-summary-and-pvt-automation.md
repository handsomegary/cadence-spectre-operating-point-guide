...
本章整合 five-transistor OTA 的 nominal baseline，并把下一阶段 characterization
整理成可重用的 deterministic process-corner workflow。

原始笔记包含本机路径、project name、process-file details、generated filenames 与
machine-specific script settings。公开版保留工程方法，并用占位符替换可能涉及隐私的
环境信息。
...
2. DC、AC、STB、transient、noise、CMRR 与 PSRR summary metrics
...
任何 schematic sizing、bias、model、load 或 netlist 变更，都会使 frozen baseline 失效，
需要重跑受影响的 analyses。
...
最小、output resistance 最低，因此是 headroom、CMRR 与 negative-rail coupling 的重要
限制点。
...
低端 transition 在 `0.710 V` 与 `0.720 V` 之间。高端碰到 sweep boundary，因此请写成：

```text
VCM high >= 1.200 V within the tested sweep
```

除非用更宽的 sweep 证明，否则不要把 `1.200 V` 当成真正高端失效点。
...
以 center small-signal slope 为基准的 secant-gain deviation：
...
Saturation range 与 linear range 是不同规格。元件仍在 saturation，不代表 transfer curve
...
STB phase 在 sweep 范围内没有到达 Cadence gain-margin calculation 需要的 crossing。
这种情况下 `nan` gain margin 本身不代表 simulation failure 或 instability。

STB crossover 与 open-loop differential UGF 使用不同 testbench/result definition，
所以 `894.29 MHz` 与 `949.53 MHz` 不必完全相同。Unity-feedback stability 请采用 formal
STB phase margin。
...
从 100 mV 到 140 mV，output step 几乎同比增加，rise/fall time 与 settling time 接近，
maximum slope 仍增加：
...
本文件的 PSRR- fixture 使用 local VSS node，而 VDD、inputs、bias 与 load 仍参考
global `0`。VSS-tracking bias system 需要另做独立 PSRR- analysis。

## 8. Cross-Checks

实用 consistency checks：

1. VID center DC gain 是 `29.948132 dB`。
2. Open-loop low-frequency AC gain 是 `29.948160 dB`。
3. 差异约 `0.000028 dB`。
4. Differential AC、noise、CMRR、PSRR+ 与 PSRR- 都有 `1101` 个 numeric frequency
   points，范围为 1 Hz 到 100 GHz。
5. 每个 paired post-processor 都会先验证 frequency grid 再合并曲线。
6. Input-pair current sum 与 tail current 只差 `0.420 nA`。

这些检查让 baseline 足够 coherent，可以先 freeze 再进入 process-corner automation。

## 9. Engineering Assessment

Strengths：

1. Nominal power 低，`1.2 V` 下约 `167.2 uW`。
2. 相对 current 的 bandwidth 高，`139.3 uA` 下约 `949.5 MHz` UGF。
3. Nominal unity-feedback stability 强，phase margin 为 `79.63 deg`。
4. Sub-nanosecond transient response 快，overshoot 低。
5. 到测试的 `140 mV` input step 为止，未观察到 hard slew-rate ceiling。
6. Nominal device matching 与 current balance 良好。

Primary limitations：

1. Open-loop gain 偏低，约 `29.95 dB`。
2. Low-frequency CMRR 只有约 `57.85 dB`。
3. Low-frequency formal PSRR 只有约 `29.95 dB`。
4. PSRR- 在高频比 PSRR+ 更快恶化。
5. Flicker-noise corner 偏高，约 `13.86 MHz`。
6. Tail device nominal saturation margin 只有约 `67.1 mV`，且 output resistance 低。
7. 严格 0.1 percent open-loop VID linearity 只有约 `+/-0.1 mV`。

## 10. PVT Automation Flow

执行大型 matrix 前：

1. Freeze and hash golden nominal netlist 与所有 baseline scripts。
2. 从 PDK documentation 确认支持的 process-section names。
3. 依 design specification 定义 voltage 与 temperature ranges。
4. 先对 process matrix 跑 DC operating point、open-loop AC 与 STB。
5. 再选 worst corners 重跑 transient、noise、CMRR 与 PSRR。
6. Deterministic PVT 稳定后，再做 load sweep 与 Monte Carlo mismatch。
...
Coarse VID sweep 适合找范围；near-zero linearity 可能需要更细 step：
...
当 coarse step 太大时，请用 refined results 建立最终 0.1 percent 或 1 percent VID
linearity table。

## 12. Transient and Report Post-Processing

从 completed corner raw files 提取 transient metrics：
...
Generated reports 在发布前必须保留为 private，直到 paths、PDK names、machine names 与
...
Baseline 已准备好进入 deterministic PVT。接着看
[第 12 章](12-deterministic-pvt-automation.md)，依序执行 `prepare`、`core` 与
`full` modes，验证 generated OCEAN scripts，细化 VID linearity，提取 transient
metrics，并建立 consolidated process-corner report。
